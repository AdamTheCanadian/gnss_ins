...
//  ImuErrorModel imu_model_converted = imu_error_model;
//
//  Vec3d_Scale(&imu_model_converted.accelerometer_biases, micro_g_to_metres_per_second_squared);
//  Vec3d_Scale(&imu_model_converted.gyro_biases, deg_to_rad / 3600.0);
//
//  Mat3d_Scale(&imu_model_converted.accelerometer_m, 1.0e-6);
//  Mat3d_Scale(&imu_model_converted.gyro_m, 1.0e-6);
//  Mat3d_Scale(&imu_model_converted.gyro_g, (deg_to_rad) / (3600 * 9.80665));
//
//  imu_model_converted.accelerometer_noise *= micro_g_to_metres_per_second_squared;
//  imu_model_converted.gyro_noise *= deg_to_rad / 60;
//
//  EulerAngles orientation_angles = {
//      DegToRad(initial_orientation.x),
//      DegToRad(initial_orientation.y),
//      DegToRad(initial_orientation.z)
//  };
//
//  RotationMatrix body_to_nav_rotation;
//  EulerToRotation(&orientation_angles, &body_to_nav_rotation);
//
//  /* Initialize the nav solution using the true solution and the initialization errors */
//  PosLLH starting_position = {
//      .latitude_radians = DegToRad(initial_position.x),
//      .longitude_radians = DegToRad(initial_position.y),
...
//  };
//
//  EulerAngles orientation_error = {
//      .x_radians = DegToRad(initial_orientation_error.x * -1),
//      .y_radians = DegToRad(initial_orientation_error.y * -1),
//      .z_radians = DegToRad(initial_orientation_error.z * -1)
//  };
//  RotationMatrix rotation_error;
//  EulerToRotation(&orientation_error, &rotation_error);
//
//  RotationMatrix tmp = Mat3d_MultiplyMat3d(&rotation_error, &body_to_nav_rotation);
//  body_to_nav_rotation = tmp;
//
//
//
//}